...
function renderHome(catalogue = cafes) {
  return render(
    <MemoryRouter>
      <DiscoveryHome cafes={catalogue} />
      <LocationProbe />
    </MemoryRouter>,
  );
}
...
describe("DiscoveryHome", () => {
  it("renders the agreed editorial discovery surfaces", () => {
    ...
    expect(container.querySelectorAll(".discovery-home > .scene")).toHaveLength(
      7,
    );
    ...
    expect(
      screen.getByRole("group", { name: "What does today need?" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { level: 2, name: "What does today need?" }),
    ).toBeInTheDocument();
    for (const name of [
    ...
    expect(
      screen.getByRole("region", { name: "Toronto café map" }),
    ).toBeInTheDocument();
  });
  ...
  it("uses a mood choice to scope browse and roulette links", async () => {
    ...
    const browse = screen.getByRole("link", {
      name: /browse .* serious coffee/i,
    });
    const roulette = screen.getByRole("link", {
      name: /roulette .* serious coffee/i,
    });
  ...
  it("models matcha as an offering and links to real matches", async () => {
    ...
    await user.click(
      screen.getByRole("radio", { name: "Matcha and pastries" }),
    );

    const browse = screen.getByRole("link", {
      name: /browse .* matcha and pastries/i,
    });
    const roulette = screen.getByRole("link", {
      name: /roulette .* matcha and pastries/i,
    });
  ...
  it("keeps a semantic café index available alongside the map", () => {
    ...
    expect(
      within(index).getByRole("link", { name: "View Larry's Place" }),
    ).toHaveAttribute("href", "/cafes/larrys-place-parkdale");
    expect(
      within(index).getAllByRole("link", { name: /directions/i })[0],
    ).toHaveAttribute("target", "_blank");
  });

  it("renders a non-focusable trail empty state when no cafés are available", () => {
    const { container } = renderHome([]);

    expect(screen.getByText("No places are on the trail yet.")).toBeVisible();
    expect(
      container.querySelector(".city-trail__scroller"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("region", { name: "Places people keep mentioning" }),
    ).not.toBeInTheDocument();
  });
